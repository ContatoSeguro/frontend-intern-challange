...
class MyCrud{
    ...
    _clear(){
        //Método que limpa a _table. Utilizar pelo console do navegador
        this._table = null;
        localStorage.removeItem("tableCrud");
        location.reload();
    }
    ...
    _read(start, max, field = null, search = ""){
    ...
    _delete(i){
        ...
        if(this._table.hasOwnProperty(i)){
...
class View{
    ...
    list(tbl, start = 0, field = null, search = ""){
        ...
        tbl.textContent = "";
        let lst = this.crud._read(start, Infinity, field, search);
    ...
    delete(btn){
        ...
        this.crud._delete(btn.getAttribute("value"));
        window.dispatchEvent(new Event("load"));
    }
}
